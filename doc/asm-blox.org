...
** Stack Cells
   ...
   - ~inputPorts~: multiple input port specifications.  Can not be
     used with ~inputPort~.
...
** Heap Cells
   ...
   - ~readPort~ (out): from this port you can read the value at the
     ...
     -999.
   - ~writePort~ (in): writing a value to this port will store a value
     at the current address.  After writing a value, the current
     address will increase by one.
   - ~offsetPort~ (out): from this port you can read the current
     offset into memory.
   - ~seekPort~ (in): by writing a value to this port, you can set the
     offset into memory.
   - ~peekPort~ (out): this port will have the data at the current
     offset.  Reading from this port will not move the offset.
   - ~setPort~ (in): this port will set the data at the current
